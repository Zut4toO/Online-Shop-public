...
import PayPalGuestButton from '../PayPalGuestButton';
...
const CheckoutGuest = () => {
  ...
  const guestData = cartstate.guestData;
  ...
  const guestCashTranSuccess = async () => {
    ...
    if (sliderDeliveryValue === 'lieferung') {
      ...
      setTimeout(() => {
        ...
        toast.success(
          'Bestellung erfolgreich. Weitere Details erhälst du per Mail',
          {
            position: toast.POSITION.BOTTOM_RIGHT,
          }
        );
      ...
      <div className={`${loading ? 'hidden' : ''} h-[350px]`}>
        ...
        <div className='flex justify-center'>
          {sliderPaymentValue === '' ? (
            ...
            <div onClick={guestCashTranSuccess}>
              <CashButton />
            </div>
          ) : (
            <div>
