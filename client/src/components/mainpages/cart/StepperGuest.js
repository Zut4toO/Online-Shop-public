...
const Stepper = ({ stepsGuest, currentStepGuest }) => {
  ...
  const updateStep = (stepNumber, stepsGuest) => {
    const newSteps = [...stepsGuest];
    let count = 0;
  ...
  useEffect(() => {
    // create object
    const stepsState = stepsGuest.map((step, index) =>
      Object.assign(
        {},
        {
          description: step,
    ...
    const current = updateStep(currentStepGuest - 1, stepRef.current);
    setNewStep(current);
  }, [stepsGuest, currentStepGuest]);
